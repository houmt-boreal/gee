...
aoi = ee.Geometry.Rectangle([[85.6, 46.2],[91.3,49.5]]) # altai

#aoi = ee.Geometry.Rectangle([[89.6, 47.2], [90.5, 47.5]])  #
...
def addNDVI(img):
    #b = ee.Image(img).select(["B1", "B2", "B3", "B4", "B5", "B7"])
    index = img.normalizedDifference(['SR_B4', 'SR_B3']).select([0], ['NDVI']).set('system:time_start',
...
def getSRcollection(year, startDay, endDay, sensor, aoi):
    ...
    #                     .map(scaleBands).map(valuefilter))
        ...
        srCollection = (ee.ImageCollection('LANDSAT/' + sensor + '/C02/T1_L2')
                        ...
                        #.filter(ee.Filter.eq('system:index', 'LT05_194011_20040723').Not())  # rastigaisa error geometric
                        .map(scaleBands).map(valuefilter))
...
def buildMosaic(year, startDay, endDay, aoi, dummyCollection):
    ...
    img = medoidMosaic(collection, dummyCollection).set('system:time_start',
                                                        int(time.mktime(datetime.date(year, 10, 1).timetuple())))
    return ee.Image(img)
...
def buildMosaicCollection(startYear, endYear, startDay, endDay, aoi, dummyCollection):
    ...
    for year in range(startYear, endYear + 1):
        ...
        imgs.append(tmp.set('system:time_start', int(time.mktime(datetime.date(year, 10, 1).timetuple()))))
...
def buildClearPixelCountCollection(startYear, endYear, startDay, endDay, aoi, dummyCollection):
    ...
    for year in range(startYear, endYear + 1):
        ...
        imgs.append(notMaskCount.set('system:time_start', int(time.mktime(datetime.date(year, 10, 1).timetuple()))))
    return ee.ImageCollection(imgs)
...
if count == count2:
    for i in range(count):
        ...
        if theMax.getInfo().get('NDVI') is not None:  # check if theMax.getInfo().get('NDVI') is none
            ...
            filename = crs_raw + '_' + 'autumn_medoid_comp_' + str(int(aa) + startYear) + '.tif'
            geemap.ee_export_image_to_drive(img.select('NDVI'), description=filename, folder='north_l7_cloudnnew2',
                                            region=aoi, scale=30)

            filename2 = crs_raw + '_' + 'autumn_medoid_comp_clearcount_' + str(int(aa2) + startYear) + '.tif'
            geemap.ee_export_image_to_drive(img_clear, description=filename2, folder='north_l7_cloudnnew2', region=aoi,
                                            scale=30)
