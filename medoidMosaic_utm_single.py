...
aoi = ee.Geometry.Rectangle([[26.07, 69.94], [26.35, 70.06]])  # rastigasa
# aoi = ee.Geometry.Rectangle([[85.6, 46.2],[90.3,49.5]]) # altai_west
# aoi = ee.Geometry.Rectangle([[89.6, 47.2],[90.9,47.5]])
...
def getSRcollection(year, startDay, endDay, sensor, aoi):
    ...
    # if srCollection.size().getInfo()>0:
    #     img_proj_based= srCollection.first()
    #     print (img_proj_based.getInfo())
    ...
    def prepImages(img):
        ...
        # print(MappedWater.projection().getInfo()['crs'])    # output EPSG:4326
...
def medoidMosaic(inCollection, dummyCollection):
    ...
    return ee.ImageCollection(difFromMedian).reduce(ee.Reducer.min(7)).select([1, 2, 3, 4, 5, 6],
                                                                              ['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4',
                                                                               'SR_B5', 'SR_B7']).reproject(
        img_proj_based.projection())
...
if count == count2:
    for i in range(count):
        img = ee.Image(imageList.get(i))  #   projection from line 312
        crs_raw = img.projection().getInfo()['crs']
        aa = img.getInfo()['properties']['system:index']

        print (img.getInfo())

        # aa = img.getInfo()['properties']['system:time_start']
        # cc = pd.to_datetime(aa, unit='ms').strftime('%Y-%m-%d')

        img_clear = ee.Image(imageList2.get(i)).reproject(img.projection())
        ...
        imgtest_just = ee.Image(imageList.get(i)).reproject('EPSG:32635', None, 30)
        theMax = imgtest_just.select('NDVI').reduceRegion(ee.Reducer.max(), aoi)
        ...
        if theMax.getInfo().get('NDVI') is not None:  # check if theMax.getInfo().get('NDVI') is none
            ...
            geemap.ee_export_image_to_drive(img.select('NDVI'), description=filename, folder='north_l7_cloudnnew13slcoff',
                                            region=aoi, scale=30)

            filename2 = crs_raw + '_' + 'medoid_comp_clearcount_' + str(int(aa2) + startYear) + '.tif'
            geemap.ee_export_image_to_drive(img_clear, description=filename2, folder='north_l7_cloudnnew13slcoff', region=aoi,
                                            scale=30)
